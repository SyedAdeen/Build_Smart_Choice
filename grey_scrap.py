    ...
    try:
        ...
        for i in range(6,7):
            ...
            if numeric_part:
                ...
                return price_sevarage_6dia
            else:
                print("No numeric value found of Severage 6 Dia Pipe")   
...
def get_water_transfer_price(url):
    try:
        ...
        if numeric_part:
            ...
            return price_float_water_transfer
        else:
...
def main():
    ...
    "Roof Insulation",
    "Termite",
    "Excavation & Backfill + Ghassu",
    "Arma Flex 32 mm o/d Pipe",
    "Arma Flex 50 mm o/d Pipe",
    "Steel",
    "Hot and Cold Water 25 mm o/d Pipe",
    "Hot and Cold Water 32 mm o/d Pipe",
    "Hot and Cold Water 40 mm o/d Pipe",
    "Hot and Cold Water 50 mm o/d Pipe",
    "Drainage 2 Dia Pipe",
    "Drainage 3 Dia Pipe",
    "Drainage 4 Dia Pipe",
    "Severage 6 dia Pipe",
    "1.5 mm Copper Conductor Wire",
    "2.5 mm Copper Conductor Wire",
    "4 mm Copper Conductor Wire",
    "GI Welded Pipe",
    "Gas Cock",
    "Water Pump",
    "Water Transfer",
    "Floor Drain",
    "AC Drainage",
    "Four Core 16 mm Conducting Cable",
    "Telephone Cable",
    "Coaxial Cable"
    ]
    ...
    brick_price = get_brick_price(url_brick)
    prices.append(brick_price)
    
    cement_price = get_cement_price(url_cement)
    prices.append(cement_price)

    sand_price = get_sand_price(url_sand)
    prices.append(sand_price)

    crush_price = get_crush_price(url_crush)
    prices.append(crush_price)


    roof_insl_price = get_roof_insl_price(url_roof_insl)
 
    for i in range (0,2):
        prices.append(roof_insl_price[i])
        
    
    termite_price = get_termite_price(url_termite)
    prices.append(termite_price)


    excavation_and_armaflex_price = get_excavation_and_armaflex_price(url_excavation)

    for i in range(0,3):
        prices.append(excavation_and_armaflex_price[i])


    steel_price = get_steel_price(url_steel)
    prices.append(steel_price)

    pipe_price = get_pipe_price(urls_pipes)
    for i in range(0,4):
        prices.append(pipe_price[i])
    
    dia_pipes_price = scrap_dia_pipes(url_dia_pipes)
    for i in range(0,3):
        prices.append(dia_pipes_price[i])
    
    sevarage_6dia_price = scrap_sevarage_6dia(url_severage_6dia)
    prices.append(sevarage_6dia_price)

    copper_wire_prices = copper_wire_price(url_copper_wires)
    for i in range(0,3):
        prices.append(copper_wire_prices[i])


    gi_welded_pipe_prices = gi_welded_pipe_price(url_gi_welded)
    prices.append(gi_welded_pipe_prices)
    
    gas_cock_price = get_gas_cock_price(url_gas_cock)
    prices.append(gas_cock_price)
    
    water_pump_price = get_water_pump_price(url_water_pump)
    prices.append(water_pump_price)

    water_transfer_price = get_water_transfer_price(url_water_transfer)
    prices.append(water_transfer_price)
    
    floor_drain_price = get_floor_drain_price(url_floor_drain)
    prices.append(floor_drain_price)

    ac_drainage_price = get_ac_drainage_price(url_ac_drainage)
    prices.append(ac_drainage_price)
    
    four_core_16_mm_prices = four_core_16_mm_price(url_four_core_16mmm)
    prices.append(four_core_16_mm_prices)
    
    phn_cable_price = get_phn_cable_price(url_telephone_cable)
    prices.append(phn_cable_price)

    coxl_price = get_coxl_price(url_coaxial_cable)
    prices.append(coxl_price)

    # for material,rates,factor in zip(materials,prices,factors_list):

    #     print(material +" -  Rs: "+ str(rates)+ " "+factor)
